...
export const attachDebuggerExecutableLaunchProfile = (resource: string, profileName: string) => vscode.l10n.t('{0} runs through the \'{1}\' launch profile, which uses commandName \'Executable\', so the running process is the one that profile starts rather than the project\'s own output. Attach to it from the Run and Debug view instead.', resource, profileName);
export const attachDebuggerUnresolvedLaunchProfile = (resource: string, profileName: string) => vscode.l10n.t('{0} was started with the \'{1}\' launch profile, which is no longer readable from its launchSettings.json, so what the running process is cannot be determined. Restore the profile or attach from the Run and Debug view instead.', resource, profileName);

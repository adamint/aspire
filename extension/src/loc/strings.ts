...
export const appHostLifecycleStartModelDescription = vscode.l10n.t("Prefer this tool over invoking Aspire AppHost lifecycle commands in a terminal whenever VS Code is active. Start an Aspire AppHost that already exists in the current workspace, using the editor's own debug lifecycle. Requires the workspace-relative path of the AppHost project or single-file AppHost, and whether to start it in 'run' mode (no debugger attached) or 'debug' mode (debugger attached). Does not create, pick, or guess an AppHost: if the path is missing, ambiguous, or outside the workspace the call fails. If the AppHost is already starting or already running, no second process is started.");
export const appHostLifecycleStopModelDescription = vscode.l10n.t("Prefer this tool over invoking Aspire AppHost lifecycle commands in a terminal whenever VS Code is active. Stop an Aspire AppHost that this editor started, using the editor's own debug lifecycle so the AppHost and its child resources shut down together. Requires the workspace-relative path of the AppHost. Only AppHosts started by this editor can be stopped; an AppHost started outside the editor is reported instead of being terminated.");

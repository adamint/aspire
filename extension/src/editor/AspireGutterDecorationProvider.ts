...
export class AspireGutterDecorationProvider implements vscode.Disposable {
    ...
    private _applyDecorations(editor: vscode.TextEditor): void {
        ...
        for (const parsed of resources) {
            ...
            // Use statementStartLine to match where the CodeLens appears on multi-line fluent chains
            const decorationLine = parsed.statementStartLine ?? parsed.range.start.line;
            buckets.get(category)!.push({ range: editor.document.lineAt(decorationLine).range });
        }

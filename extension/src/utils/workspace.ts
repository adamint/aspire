...
function tryParseAppHostCandidate(obj: unknown, rootFolder: vscode.WorkspaceFolder): AppHostCandidate | undefined {
    if (!obj || typeof obj !== 'object') {
        return undefined;
    }

    const candidate = obj as Partial<AppHostCandidate>;
    ...
    return {
        relativePath: typeof candidate.relativePath === 'string'
            ? candidate.relativePath
            : path.relative(rootFolder.uri.fsPath, candidate.path),
        path: candidate.path,
...
function parseAppHostCandidates(stdout: string, rootFolder: vscode.WorkspaceFolder): ParsedAppHostCandidates {
    const parsed = JSON.parse(stdout);
    if (Array.isArray(parsed)) {
        return {
            candidates: parsed.flatMap(candidate => {
                const parsedCandidate = tryParseAppHostCandidate(candidate, rootFolder);
    ...
    const result = new Promise<AppHostProjectSearchResult>((resolve, reject) => {
        ...
        proc = spawnCliProcess(terminalProvider, cliPath, args, {
            ...
            exitCallback: async code => {
                ...
                try {
                    const parsed = parseAppHostCandidates(stdout, rootFolder);

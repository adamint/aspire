...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const debuggerInstallHintService = new DebuggerInstallHintService(context.globalState);
  const notifyMissingDebuggers = () => {
    const resources = [
      ...dataRepository.workspaceResources,
      ...dataRepository.appHosts.flatMap(appHost => appHost.resources ?? []),
    ];

    ...
    static async start(): Promise<DcpServer> {
        return new Promise((resolve, reject) => {
            ...
            app.put('/run_session', requireHeaders, async (req: Request, res: Response) => {
                ...
                const command = payload.env?.find(env => env.name === 'EXECUTABLE_COMMAND_NAME')?.value;
                if (!command) {
                    const error: ErrorDetails = {
                        code: 'MissingCommand',
                        message: 'EXECUTABLE_COMMAND_NAME environment variable is required.',
                        details: []
                    };
                    ...
                    for (const launchConfig of payload.launch_configurations) {
                        if (launchConfig.mode === 'NoDebug') {
                            ...
                            if (launchConfig.type === "node") {
                                ...
                                throw new Error(`Unsupported command for debug mode: ${command}`);
                            }

                            extensionLogOutputChannel.info(`Debugging session created with ID: ${runId}`);

                            const debugSession = await startAndGetDebugSession(debugConfig);
                            if (!debugSession) {
                                const error: ErrorDetails = {

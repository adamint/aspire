        ...
        if (session.configuration.runId === debugConfig.runId) {
          ...
          const stopSession = () => {
            if (browserTermination) {
              extensionLogOutputChannel.info(`Stopping browser debug session: ${session.name} (run id: ${session.configuration.runId})`);
              return browserTermination.stop();
            }
          ...
          this._disposables.push({
            dispose: browserTermination
              ? () => browserTermination.stopAndDisposeOnFailure()
              : stopSession
          });

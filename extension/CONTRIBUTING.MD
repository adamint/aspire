...
> **Heads up about the internal npm mirror.** The repo's `.npmrc` routes all package downloads through the dnceng `dotnet-public-npm` Azure Artifacts feed, which is a pull-through cache of npmjs.org. The first time anyone (locally or in CI) asks the feed for a Yarn release that has never been requested, the feed has to fetch it from npmjs.org — and anonymous pull-through fetches fail with HTTP 401. After the first authenticated request the tarball is cached forever. If you bump the Yarn version, either ensure CI has run once with credentials, or pre-seed the feed by running an authenticated `npm install yarn@<new>` against the `dotnet-public-npm` registry yourself.

...
const enableDebuggerExtensions = enableAzureFunctionsE2E || enableBrowserDebuggerE2E;
...
function resolveDebuggerVsixPaths() {
  ...
  // Managed browser debugging and Azure Functions both depend on C#, which activates the .NET
  // runtime extension directly. Azure's extensions stay limited to the Functions shard.
  const paths = [
    {
...
function resolveRequiredVsixPath(environmentVariable) {
  ...
  if (!configuredPath) {
    throw new Error(`${environmentVariable} is required when ASPIRE_EXTENSION_E2E_ENABLE_AZURE_FUNCTIONS=true or ASPIRE_EXTENSION_E2E_SHARD=browser-debugger.`);
  }

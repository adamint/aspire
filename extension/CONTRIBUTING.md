...
corepack yarn watch     # incremental webpack build; leave running while you edit
```

Open `extension/` in VS Code and launch **Run Extension** (`F5`) to start an Extension Development Host with your build. After the watch task rebuilds, re-launch or run **Developer: Reload Window** in the host to pick up changes.

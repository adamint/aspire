...
On Linux, run the E2E command under `xvfb-run -a` when no desktop session is available. On Windows, `ASPIRE_EXTENSION_E2E_CLI_PATH` can point at an `.exe` or `.cmd` wrapper, including paths with spaces. Set `ASPIRE_EXTENSION_E2E_VSIX=/path/to/aspire-extension.vsix` to test an existing package instead of letting the runner create one. The runner defaults to VS Code 1.122.1 and ExTester 8.23.0 installed from `dotnet-public-npm`; override with `ASPIRE_EXTENSION_E2E_VSCODE_VERSION` or `ASPIRE_EXTENSION_E2E_EXTESTER_VERSION` when you need to investigate version-specific behavior. The VS Code user data is forced to English (`locale.json` plus `VSCODE_NLS_CONFIG`) so UI text assertions are deterministic across machines.
...
Linux CI shards also record the Xvfb display with `ffmpeg`. The default workflow mode is `ASPIRE_EXTENSION_E2E_RECORDING_MODE=failure`, which records during the run but only keeps and uploads `extension/.test-recordings/<shard>/*.mp4` when the shard fails. Set `ASPIRE_EXTENSION_E2E_RECORDING_MODE=always` for a manual run when you need a successful-run recording, or `off` to disable recording. The default capture size is `1280x1024`, matching the hosted Xvfb display; override with `ASPIRE_EXTENSION_E2E_RECORDING_SIZE` if you run under a larger display. Recording is intentionally Linux-only because the Windows E2E jobs do not run under Xvfb and hosted desktop capture is less reliable.

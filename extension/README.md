...
- **Dashboard telemetry passthrough** — when the extension hosts the Aspire dashboard, telemetry events emitted by the dashboard are forwarded through the extension's reporter under a small fixed set of event names (`dashboard/operation`, `dashboard/userTask`, `dashboard/fault`, `dashboard/asset`, `dashboard/scope/start`, `dashboard/scope/end`, `dashboard/property/set`, `dashboard/property/recurring`, `dashboard/commandLineFlags`). The original dashboard-side event name is carried as a `dashboard_event_name` property, and the per-event property/metric bag is JSON-encoded into two stable properties (`dashboard_properties`, `dashboard_measurements`) capped at 100 entries and 8 KB each — so the upstream dashboard adding new events or properties does not silently grow the extension's classification surface. Properties the dashboard explicitly tags as personally identifiable are dropped before forwarding. Every string value in the bundle (including dashboard-supplied properties on fault events such as exception messages and stack traces) is additionally capped per-entry at ~1 KB before bundling so that a single property cannot serve as a side channel for multi-KB workspace content (paths, traces). Bundle keys and other dashboard-supplied identifiers forwarded as first-class fields (event names, property names, asset ids, command-line flag prefixes) are clamped to 256 characters as defense-in-depth against an upstream regression that puts user-controlled strings into identifier positions. Free-form diagnostic strings the dashboard surfaces as first-class fields on failure events (top-level error messages and fault descriptions) are similarly truncated and routed through VS Code's telemetry reporter, which performs additional pattern-based scrubbing for common PII shapes (home-directory paths, emails, well-known tokens) — but because the dashboard composes these strings from runtime data, it is possible for user-controlled fragments (for example, a resource name) to be included in a forwarded exception message. If this concerns you, disable telemetry at the VS Code level.

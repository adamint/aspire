...
- **Dashboard telemetry passthrough** — when the extension hosts the Aspire dashboard, telemetry events emitted by the dashboard are forwarded through the extension's reporter using the same global VS Code telemetry setting. Properties the dashboard explicitly tags as personally identifiable are dropped before forwarding. Free-form diagnostic strings the dashboard may include in failure events (error messages, fault descriptions) are truncated and routed through VS Code's telemetry reporter, which performs additional pattern-based scrubbing for common PII shapes (home-directory paths, emails, well-known tokens) — but because the dashboard composes these strings from runtime data, it is possible for user-controlled fragments (for example, a resource name) to be included in a forwarded exception message. If this concerns you, disable telemetry at the VS Code level.

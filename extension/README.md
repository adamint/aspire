...
Both tools require the workspace-relative path of the apphost — absolute paths are rejected — ask a chat agent's user to confirm before doing anything, and only work in a [trusted workspace](https://code.visualstudio.com/docs/editing/workspaces/workspace-trust). The path shown in that confirmation is the exact path the tool will act on. They never pick an apphost for you: if the path is missing, ambiguous, or outside your workspace the call fails instead of guessing. Starting an apphost that is already starting or running does not launch a second one, and stopping only works for apphosts this editor started — an apphost you started from a terminal is reported back rather than terminated. If the extension cannot determine whether an apphost is running outside the editor, the call reports a failure rather than claiming nothing is running.

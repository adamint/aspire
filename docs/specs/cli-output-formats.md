...
The Marketplace is only queried when a parseable [semantic version](https://semver.org) was resolved, update notifications are enabled, and the extension is known to come from the VS Code Marketplace. An install is treated as a Marketplace install when VS Code recorded its origin as `gallery`. That origin is read from the profile's `extensions.json`, which records it for every installed extension; the extracted `package.json` is only consulted for entries that index does not list, and there a publisher id stamped into the scanner-owned metadata is accepted as the weaker signal. The extension host cannot see the origin at all — VS Code strips that metadata from the description an extension reads — so `ASPIRE_VSCODE_EXTENSION_SOURCE` normally reports `unknown`; the check then resolves the origin and channel from disk, and adopts them only when the disk scan resolved the same version the extension reported, which is what ties the record to the running instance rather than to a second copy in another root. A non-Marketplace or unknown-source install reports the extension as installed without an update comparison. The explicit channel signal selects the Marketplace release channel because Marketplace pre-release extensions use plain `major.minor.patch` versions rather than semver pre-release tags; when the channel is `unknown` the check reports the lookup as unavailable instead of comparing against a guessed feed. A successful lookup sets `latestVersionKnown` to `true` and adds `latestVersion` (string), `latestVersionChannel` (`stable` or `pre-release`), and `updateAvailable` (bool). A lookup that times out, is unavailable, or omits the requested channel leaves `latestVersionKnown` as `false`, adds `latestVersionError` (`timeout` or `unavailable`), and reports the reason in `details` without a `fix` or `link`.

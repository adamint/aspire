  ...
  "checks": [
    ...
    },
    {
      ...
      "metadata": {
        ...
        "extensionVersionSource": "extension",
        "extensionChannel": "stable",
        "extensionInstallSource": "marketplace",
...
Its `metadata` always exposes `vsCodeInstalled` (bool), `extensionInstalled` (bool), and `extensionId` (string). An installed extension adds `updateCheckEnabled` (bool), `latestVersionKnown` (bool), `extensionVersionSource` (`extension`, `manifest`, or `unknown`), `extensionChannel` (`stable`, `pre-release`, or `unknown`), `extensionInstallSource` (`marketplace` or `unknown`), and `extensionVersion` (string) when a version was resolved. When `updateCheckEnabled` is `true` it also adds `extensionVersionKnown` (bool), which is `true` when the installed version parsed as a semantic version and `false` when it could not be resolved. `extensionVersionKnown` is absent when update checking is disabled, because no version comparison was attempted.

The installed version is resolved in two steps. The `ASPIRE_VSCODE_EXTENSION_VERSION`, `ASPIRE_VSCODE_EXTENSION_CHANNEL`, and `ASPIRE_VSCODE_EXTENSION_SOURCE` environment variables, which the Aspire VS Code extension contributes to the terminals VS Code creates for it (and therefore to tasks, and to debug sessions configured with `"console": "integratedTerminal"`, because those run inside a terminal), are authoritative and used first (`extensionVersionSource` is `extension`): only the extension host knows which installation is loaded, because several extension roots can hold the extension at once (a desktop install plus `.vscode-server` for Remote, WSL, and dev containers), `--extensions-dir` is invisible to a child process, and portable mode relocates the whole root. When the variable is absent — an extension predating the variable, or a doctor run outside a VS Code-created process — the check scans the known extension roots and reads the `version` field of each installed extension's `package.json`, falling back to the `<publisher>.<name>-<version>` folder name, and takes the highest version by semver precedence within one root (`extensionVersionSource` is `manifest`). A folder the profile's `extensions.json` lists outranks one it does not, whatever their versions, because that index names the folder VS Code actually loads and an interrupted update can leave a higher-numbered folder behind that is never loaded. An index entry with no `metadata` still names its folder, and its channel and origin fall back to whatever the extracted manifest retained (a manifest with no pre-release flag reads as `stable`); the index only overrides the manifest where it carries a signal of its own. If more than one default root contains the Aspire extension and no environment signal identifies the active root, the check reports the version as unknown rather than taking a cross-root maximum. If neither step yields a version the check reports a distinct `warning` naming the roots it searched and sets `extensionVersionKnown` to `false`; it never reports `pass` on an undetermined version. A non-empty but unparseable `ASPIRE_VSCODE_EXTENSION_VERSION` counts as proof that the extension is installed, because only the running extension sets that variable and its installation can live outside the roots the CLI can enumerate. Nothing the disk scan found is adopted in that case: the variable being present at all means a specific instance is loaded, and it not parsing means the CLI cannot say which, so a stale copy under a default root would otherwise donate its version, channel, and origin to a portable or `--extensions-dir` instance. The scan still supplies the roots the warning names.

The Marketplace is only queried when a parseable [semantic version](https://semver.org) was resolved, update notifications are enabled, and the extension is known to come from the VS Code Marketplace. An install is treated as a Marketplace install when VS Code recorded its origin as `gallery` **and** the build that owns the extension root installs from the VS Code Marketplace. A root named by `VSCODE_EXTENSIONS` identifies a directory rather than a product — VS Code, VSCodium, and code-oss all honor the variable, and any of them can be pointed at any directory — so an overridden root never resolves to a Marketplace origin, whatever it is named. `gallery` names whichever gallery the product is configured against, and VSCodium points `extensionsGallery` at Open VSX, so a `gallery` install under one of its roots resolves to an unknown origin rather than being compared against or linked to the Marketplace. That origin is read from the default profile's `extensions.json`, which records it for every extension installed into that profile; the extracted `package.json` is only consulted for entries that index does not list, and there only a `source` VS Code wrote is accepted — a publisher id is not, because VS Code looks a side-loaded VSIX up in the gallery on install and stamps the matched publisher id onto it. A `source` VS Code did write settles the question even when it is malformed. An extension installed only into a non-default profile is recorded in that profile's own index under the user-data directory, so it resolves to an unknown origin and is reported as installed without an update comparison. The extension host cannot see the origin at all — VS Code strips that metadata from the description an extension reads — so `ASPIRE_VSCODE_EXTENSION_SOURCE` normally reports `unknown`; the check then resolves the origin and channel from disk, and adopts them only when the disk scan resolved exactly the version the extension reported, build metadata included, which is what ties the record to the running instance rather than to a second copy in another root. A non-Marketplace or unknown-source install reports the extension as installed without an update comparison, and the undetermined-version `warning` omits its `link` for those installs, because the Marketplace page does not describe what is installed. The channel is the one the install tracks (`preRelease`), not the one the extracted artifact happens to be (`isPreReleaseVersion`), because opting into pre-release updates flips the tracked channel before the next update lands. The explicit channel signal selects the Marketplace release channel because Marketplace pre-release extensions use plain `major.minor.patch` versions rather than semver pre-release tags; when the channel is `unknown` the check reports the lookup as unavailable instead of comparing against a guessed feed. A successful lookup sets `latestVersionKnown` to `true` and adds `latestVersion` (string), `latestVersionChannel` (`stable` or `pre-release`), and `updateAvailable` (bool). A lookup that times out, is unavailable, or omits the requested channel leaves `latestVersionKnown` as `false`, adds `latestVersionError` (`timeout` or `unavailable`), and reports the reason in `details` without a `fix` or `link`.

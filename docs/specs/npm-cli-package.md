...
Stable releases run public npm smoke validation and internal mirror seeding/validation by default. Mirror seeding and anonymous validation are gated on `SkipNpmMirrorValidation=false`; the public smoke test is gated on the release doing npm work at all, so it still runs when a publishing run sets `SkipNpmMirrorValidation=true` and is skipped only when all three npm skips are `true`. The seed step reads `optionalDependencies` from the staged pointer tarball and uses authenticated `npm pack --ignore-scripts` calls for the pointer and every exact RID package, so platform gating cannot omit non-Windows tarballs. Anonymous validation resolves internal `@latest`, packs that exact pointer with credential-free configuration, reads its own optional dependencies, and packs every exact RID tarball before promotion. Dry runs and prereleases do not perform real mirror work. Mirror-only recovery sets both npm publish skips to `true` while leaving `SkipNpmMirrorValidation=false`; unrelated stable recovery runs set all three skips to `true`.

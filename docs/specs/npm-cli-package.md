...
The published package is `@microsoft/aspire-cli`. It supports global installation (`npm install -g @microsoft/aspire-cli`) on Windows, macOS, and Linux (glibc and musl). The package is still published only to the public npm registry. npm-installed CLI update checks resolve `@microsoft/aspire-cli@latest` through the anonymously readable `dotnet-public-npm` Azure Artifacts feed. Stable releases authenticate to that feed after the public npm smoke test and pack the exact staged pointer plus every optional RID dependency to trigger upstream ingestion. They then switch to credential-free npm configuration and caches to pack the mirrored `@latest` pointer and each of its exact optional dependencies before channel promotion.

The CLI checks how it was installed before selecting an update source. NuGet-installed or standalone launches retain the existing NuGet package lookup. npm-installed launches skip NuGet entirely and resolve `@microsoft/aspire-cli@latest` anonymously from `dotnet-public-npm` with isolated project/user/global npm configuration, a fresh cache, deterministic text output, and a short shared timeout. When that concrete version is newer than the running CLI, the notification shows `npm install -g @microsoft/aspire-cli@latest`. If the mirrored `latest` version is equal to or older than the running CLI, no update is reported. Caller cancellation only stops that caller's wait; failed or timed-out shared lookups can be retried. Routine commands suppress lookup failures, while `aspire doctor` reports a specific update-check warning, including when npm is unavailable. Explicit `aspire update --self` behavior is unchanged.
...
Stable releases run public npm smoke validation and internal mirror seeding/validation by default. Mirror seeding and anonymous validation are their own release action, `NpmInternalMirrorAction`: `auto` (the default) runs them only when the release publishes npm packages, `only` performs an intentional mirror-only rerun with both npm publish skips set, and `skip` disables the mirror steps alone. The public smoke test is never gated on `NpmInternalMirrorAction`; it runs whenever the release publishes npm packages or performs a mirror-only rerun, so a publishing run cannot promote without proving the public package installs. The seed step reads `optionalDependencies` from the staged pointer tarball and uses authenticated `npm pack --ignore-scripts` calls for the pointer and every exact RID package, so platform gating cannot omit non-Windows tarballs. Anonymous validation resolves internal `@latest`, packs that exact pointer with credential-free configuration, reads its own optional dependencies, and packs every exact RID tarball before promotion. Dry runs and prereleases do not perform real mirror work. Mirror-only recovery sets both npm publish skips to `true` and `NpmInternalMirrorAction=only`; recovery runs unrelated to npm set both npm publish skips to `true` and change nothing else, because `auto` already emits no npm steps for a run that publishes no npm packages.

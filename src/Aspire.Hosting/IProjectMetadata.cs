...
public interface IProjectMetadata : IResourceAnnotation
{
    ...
    /// <summary>
    /// Gets the assembly name that the project evaluates to, or <see langword="null"/> when it is unknown.
    /// </summary>
    /// <remarks>
    /// <para>
    /// This value is baked into the generated project metadata when the AppHost is built. It is the MSBuild-evaluated
    /// name of the built output (<c>TargetName</c>, which defaults to <c>AssemblyName</c>) rather than the project file
    /// name. That distinction matters when a project sets <c>AssemblyName</c> - often from an imported
    /// <c>Directory.Build.props</c> - because the launched assembly is then named after the assembly and not after the
    /// project.
    /// </para>
    /// <para>

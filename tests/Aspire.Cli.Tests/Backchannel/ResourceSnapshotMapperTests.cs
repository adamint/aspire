...
public class ResourceSnapshotMapperTests
{
    ...
    [Fact]
    public void MapToResourceJson_PreservesProjectAssemblyNameProperty()
    {
        ...
        var resource = new ResourceSnapshot
        {
            ...
            Properties = new Dictionary<string, JsonNode?>
            {
                ...
                ["project.assemblyName"] = JsonValue.Create("My Attach Service")
            }
        };
        ...
        Assert.Equal("My Attach Service", result.Properties["project.assemblyName"]?.GetValue<string>());

        var json = JsonSerializer.Serialize(result, ResourcesCommandJsonContext.RelaxedEscaping.ResourceJson);
        using var document = JsonDocument.Parse(json);
        Assert.Equal("My Attach Service", document.RootElement.GetProperty("properties").GetProperty("project.assemblyName").GetString());
    }

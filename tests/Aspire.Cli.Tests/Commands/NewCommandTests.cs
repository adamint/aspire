...
public class NewCommandTests(ITestOutputHelper outputHelper)
{
    ...
    [Fact]
    public async Task NewCommandCreatesVsCodeWorkspaceFiles()
    {
        using var workspace = TemporaryWorkspace.Create(outputHelper);
        var outputDir = workspace.CreateDirectory("MyApp");
        var services = CliTestHelper.CreateServiceCollection(workspace, outputHelper, options =>
        {
            options.DotNetCliRunnerFactory = (sp) =>
            {
        ...
        var extensionsJsonPath = Path.Combine(outputDir.FullName, ".vscode", "extensions.json");
        Assert.True(File.Exists(extensionsJsonPath));
        var extensionsContent = File.ReadAllText(extensionsJsonPath);
        Assert.Contains("microsoft-aspire.aspire-vscode", extensionsContent);

        var launchJsonPath = Path.Combine(outputDir.FullName, ".vscode", "launch.json");
    ...
    [Fact]
    public async Task NewCommandDoesNotOverwriteExistingVsCodeFiles()
    {
        using var workspace = TemporaryWorkspace.Create(outputHelper);
        var outputDir = workspace.CreateDirectory("MyApp2");
        ...
        File.WriteAllText(Path.Combine(vsCodeDir, "launch.json"), """{"version":"0.2.0","configurations":[]}""");

...
public sealed class ResourceMenuBuilderTests
{
    ...
    [Fact]
    public void AddMenuItems_ResourceWithParameterRelationship_ViewRelatedParametersShown()
    {
        // Arrange
        var paramResource = ModelTestHelpers.CreateResource(
        ...
        var resource = ModelTestHelpers.CreateResource(
            ...
            displayName: "myapp",
            relationships: [new RelationshipViewModel("api-key", "Reference")]);
        var resourceByName = new Dictionary<string, ResourceViewModel>(StringComparer.OrdinalIgnoreCase)
        {
            [resource.Name] = resource,
            [paramResource.Name] = paramResource,
        };
        var repository = TelemetryTestHelpers.CreateRepository();
        var aiContextProvider = new TestAIContextProvider();
        var resourceMenuBuilder = CreateResourceMenuBuilder(repository, aiContextProvider);
        ...
        // Assert - "View related parameters" should be in the menu
        Assert.Contains(menuItems, e => e.Text == "Localized:ResourcesViewRelatedParameters");
    }

    [Fact]
    public void AddMenuItems_ResourceWithoutParameterRelationship_ViewRelatedParametersNotShown()
    {
        // Arrange - resource with no parameter relationships
        var otherResource = ModelTestHelpers.CreateResource(
            resourceName: "db-abc123",
            displayName: "db",
            resourceType: KnownResourceTypes.Container);
        var resource = ModelTestHelpers.CreateResource(
            resourceName: "myapp-abc123",
            displayName: "myapp",
            relationships: [new RelationshipViewModel("db", "Reference")]);
        var resourceByName = new Dictionary<string, ResourceViewModel>(StringComparer.OrdinalIgnoreCase)
        {
            [resource.Name] = resource,
            [otherResource.Name] = otherResource,
        };
        var repository = TelemetryTestHelpers.CreateRepository();
        var aiContextProvider = new TestAIContextProvider();
        var resourceMenuBuilder = CreateResourceMenuBuilder(repository, aiContextProvider);
        ...
        // Assert - "View related parameters" should NOT be in the menu
        Assert.DoesNotContain(menuItems, e => e.Text == "Localized:ResourcesViewRelatedParameters");
    }

    [Fact]
    public void AddMenuItems_ParameterResource_ViewRelatedParametersNotShown()
    {
        // Arrange - parameter resource itself should never show "View related parameters"
        var resource = ModelTestHelpers.CreateResource(
            resourceName: "api-key",
            displayName: "api-key",
            resourceType: KnownResourceTypes.Parameter);
        ...
        // Assert - parameters themselves should NOT show "View related parameters"
        Assert.DoesNotContain(menuItems, e => e.Text == "Localized:ResourcesViewRelatedParameters");
    }

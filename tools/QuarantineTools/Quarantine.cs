...
public class Program
{
    ...
    /// <summary>
    /// Upper bound for the <c>git rev-parse</c> probe.
    /// </summary>
    private static readonly TimeSpan s_gitProbeTimeout = TimeSpan.FromSeconds(10);

    /// <summary>
    /// Upper bound for waiting after terminating a timed-out git process.
    /// </summary>
    private static readonly TimeSpan s_processCleanupTimeout = TimeSpan.FromSeconds(2);
    ...
    private static async Task<string?> TryGetGitTopLevelAsync(string startDir, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // one terminator and nothing else: POSIX allows a checkout directory name to end in a space,
            // a newline, or a carriage return, so any character before the terminator is part of the
            // path, and trimming it produces a root the wrong-tree guard then rejects as a different
            // repository.
    ...
    private static async Task TerminateProcessAsync(Process process)
    {
        ...
        {
            // Process disposal closes the remaining handles; cleanup is bounded so git cannot hang the tool.
        }
    }

    /// <summary>
    /// Returns the error text to print when <paramref name="repoRoot"/> does not match the repository
    /// marker nearest to the caller, or null when the root is safe to use.
    /// </summary>
    internal static string? TryGetWrongTreeError(string repoRoot, string currentDirectory)
    {
        var markerRoot = FindRepoRootByMarker(currentDirectory);
        var expectedRoot = markerRoot ?? currentDirectory;

        // An ancestry check is insufficient here: the config reached through the nearest `.git`
        // directory or gitfile can set core.worktree to an outer ancestor. Git then reports that
        // ancestor from --show-toplevel even though the caller selected the inner tree by standing
        // beneath its marker. Require the roots to be identical so that metadata cannot redirect a
        // bulk source rewrite across that boundary.
        //
        // Canonicalization preserves legitimate alternate spellings: a symlinked checkout or Windows
        // junction can make the marker root read as `D:\src\aspire` while git reports
        // `C:/src/aspire`. Canonicalize covers symlinks and junctions, which is what ResolveLinkTarget
        // reports. It does not resolve DOS device mappings such as `subst`, but that gap can only cause
        // a refusal because both directions must compare as the same directory.
        if (IsSameDirectory(Canonicalize(repoRoot), Canonicalize(expectedRoot)))
        {
            return null;
        }

        // Name every path involved: the whole point of this guard is that a wrong-tree run is otherwise
        // indistinguishable from a correct one, so the message has to say which boundary was rejected.
        return $"""
            Refusing to run: the resolved repository root does not match the nearest .git marker root.
              Resolved repository root: {repoRoot}
              Expected repository root: {expectedRoot}
              Current directory:        {currentDirectory}
            """;
    }

    private static bool IsSameDirectory(string first, string second)
        => IsSameOrAncestorDirectory(first, second) && IsSameOrAncestorDirectory(second, first);
    ...
    internal static bool IsSameOrAncestorDirectory(string candidateAncestor, string directory, Func<string, bool> isCaseSensitiveDirectory)
    {
        ...
        for (var i = 0; i < ancestorSegments.Length; i++)
        {
            ...
            //
            // Identical spellings match under either rule, so settle that before probing. The probe is
            // not free: off Windows it enumerates the whole parent directory, and the common case walks
            // every ancestor from the filesystem root down, so probing first would stat `/`, `/home`,
            // and each directory in between on a run where no segment differs at all.
